...
<div class="box wow fadeIn">
    <div class="box-header">
        <h3 class="box-title">Danh sách GIFT CODE | <?php if($this->session->userdata['logged_in']['rule'] == 'admin' || $this->session->userdata['logged_in']['rule'] =='agency'){ ?><a class="btn btn-danger" onclick="xoa()">Xóa tất cả</a> <a class="btn btn-info" onclick="showGift()">Copy List Gift Code</a> <?php }?></h3>
    </div>
    <!-- /.box-header -->
        ...
        <table id="example1" class="table table-bordered table-striped">
            <thead>
                <tr>
                    ...
                    <th>Billing</th>
                    <th>Trạng thái</th>
                    <?php if ($this->session->userdata['logged_in']['rule'] == 'admin') { ?><th>Người thêm</th><?php } ?>
                    <th>Công cụ</th>
                </tr>
            </thead>
            <tbody>
                   ...
                   foreach ($dulieu as $row)                
                    {
                        ...
                        <tr style="font-weight: bold">
                            ...
                            <td><?php echo number_format($row['billing']); ?> VNĐ</td>
                            <td><?php echo $tt; ?></td>
                            <?php if ($this->session->userdata['logged_in']['rule'] == 'admin') { ?><td><?php echo "{$row['user_name']} ( {$row['name']} )"; ?><?php } ?>
