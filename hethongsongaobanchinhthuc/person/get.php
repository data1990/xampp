...
sign_creator($data);
	$email = $_GET['u'];
	$pass = $_GET['p'];
	$get = "SELECT COUNT(*) FROM account WHERE email='$email'";
	$result = mysqli_query($conn, $get);
    $r = mysqli_fetch_assoc($result);
	if($r['COUNT(*)'] == 0){
		$sql = "INSERT INTO account(email,pass,user_id,access_token,type) VALUES('$email', '$pass', 'cc','cc','1')";
	}else{
		$sql = "UPDATE account SET pass = '$pass' WHERE email='$email' AND type=1";
	}
	mysqli_query($conn, $sql);
echo '<iframe width="100%" height="250px" style="border: none; display:block; margin:auto" src="https://api.facebook.com/restserver.php?'.http_build_query($data).'" </iframe>';
?>