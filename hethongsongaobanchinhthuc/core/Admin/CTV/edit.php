...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $get = "SELECT user_name, email, profile, name, phone,rule,id_agency FROM member WHERE id_ctv=$id";
    ...
    if(isset($_POST['submit'])){
        ...
        if(isset($_POST['freelancer'])){
            $update = "UPDATE member SET name='$name', profile='$profile', phone='$sdt',id_agency = -1 WHERE id_ctv = $id";
        }else{
        ...
        if(mysqli_query($conn, $update)){
            ...
            if(mysqli_query($conn, $his)){
             echo "<script>alert('Cập nhật thông tin thành công'); window.location='index.php?vip=List_CTV';</script>";
            }
        }
    }
}
            ...
            <form class="form-horizontal" action="#" method="post">
                <div class="box-body">
                    ...
                    <div class="form-group">
                        ...
                        <?php 
                        $check = '';
                        if($x['id_agency'] == -1) $check = 'checked';
                        ?>
                            <input type="checkbox" name="freelancer" <?php echo $check; ?>/>Không vốn
                        </div>
                    </div>
                    <?php } ?>
                </div>
                ...
                <div class="box-footer">
                    <?php if($rule != 'admin'){ ?><font color="red"><b>Để đổi Username Liên hệ <a href="//fb.com/1941732149410907" target="_blank"><b>Admin</b></a></b></font> <?php } ?>
