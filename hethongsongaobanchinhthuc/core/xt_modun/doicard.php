...
<?php 
    ...
    //Mã tự sinh trong mỗi giao dịch và không giống nhau (Chúng tôi sẽ lưu lại mã này để đối chiếu khi có khiếu nại) 
    $res = $api->check_card($key, $code, $serial, $request_id); 


    $getuser = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM `member` WHERE `user_name`='".$users."'")); 
    ...
    { 
        ...
        $transid    = $res->transid; //mã giao dịch 
