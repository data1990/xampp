<script>
    function check(id){
            swal({
              ...
              location.href = 'index.php?vip=Delete_Like_SV2&id_like=' + id;
            })
        }
</script>
<div class="box wow fadeIn">
    <div class="box-header">
        <h3 class="box-title">Danh sách ID VIP Cảm Xúc</h3><br> <kbd>Bạn đang thực hiện việc xem ID ở Sever 2!</kbd>
        </div>
    <!-- /.box-header -->
    <div class="box-body table-responsive">
        ...
        <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
                <?php
                if ($rule != 'admin') {
                    $get = "SELECT id,user_id, name, han, likes, max_like, start, end, pay,type FROM vipsv2 WHERE id_ctv=$idctv";
                } else if($rule == 'admin' && $idctv !=1){
                    $get = "SELECT id, user_id, vipsv2.name, han, likes, max_like, start, end, pay,type,member.rule, member.name AS ctv_name,member.user_name,rule FROM vipsv2 INNER JOIN member ON vipsv2.id_ctv = member.id_ctv WHERE vipsv2.id_ctv > 0";
                }else{
                    $get = "SELECT id, user_id, vipsv2.name, han, likes, max_like, start, end, pay,type,member.rule, member.name AS ctv_name,member.user_name,rule FROM vipsv2 INNER JOIN member ON vipsv2.id_ctv = member.id_ctv";
                }
                ...
                while ($x = mysqli_fetch_assoc($result)) {
                    ...
                    if ($rule == 'admin') {
                         if ($x['end'] >= time()) {
                           $handle = "<a href='index.php?vip=Delete_Like_SV2&id=$id' class='btn btn-info'>Cập nhật</a> <a onClick='check($id);' class='btn btn-danger'>Xóa</a>";
                        } else {
                           $handle = " <a onClick='check($id);' class='btn btn-danger'>Xóa</a>";
                         }
                    } else {
                        if ($x['end'] >= time()) {
                            $handle = "<a href='index.php?vip=Update_Like_SV2&id=$id' class='btn btn-info'>Cập nhật</a> <a onClick='check($id);' class='btn btn-danger'>Xóa</a>";
                        } else {
                            $handle = "<a onClick='check($id);' class='btn btn-danger'>Xóa</a>";
                        }
                    }
