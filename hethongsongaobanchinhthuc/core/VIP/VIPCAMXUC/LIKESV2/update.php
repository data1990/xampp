...
if (isset($_GET['id'])) {
    $id_like = $_GET['id'];
    $get = "SELECT user_id , name, likes, id_ctv, max_like,type FROM vipsv2 WHERE id=$id_like";
    ...
    if(empty($loi)){
        ...
        if($likes <= $x['max_like']){
            if($rule != 'admin' || $idctv != 1){
                $sql = "UPDATE vipsv2 SET user_id = '$user_id', name='$name', likes='$likes',type='$type' WHERE id='$id_like'";
            }else{
                $sql = "UPDATE vipsv2 SET user_id = '$user_id', name='$name', likes='$likes',max_like='$max_like',type='$type' WHERE id='$id_like'";
            }
            if (mysqli_query($conn, $sql)) {
                ...
                if($rule != 'admin' || $idctv != 1){
                    ...
                    $content = "<b>$uname</b> vừa cập nhật VIP Cảm Xúc ID <b>$uid</b> = > <b>$user_id</b>, Tên: <b>$name</b>, Số CX / Cron: <b>$likes</b> CX, Max CX: <b>$max_like</b> CX, Loại CX: <b>$type</b> tại sever <b>2</b>";
                }
                $his = "INSERT INTO history(content, time, id_ctv, type) VALUES('$content', '$time', '$idctv',0)";
                if (mysqli_query($conn, $his)) {
                    echo "<script>alert('Thành công');window.location='index.php?vip=Manager_VIP_Like_SV2';</script>";
                }
            }
        }else{
            echo "<script>alert('Vui lòng chọn số CX/Cron nhỏ hơn hoặc bằng gói CX');window.location='index.php?vip=Manager_VIP_Like_SV2';</script>";
        }
    }
}
?>
    ...
    <div class="col-md-12">
        ...
        <div class="box box-info wow fadeIn">
            <div class="box-header with-border">
                <h3 class="box-title">Cập nhật ID VIP Cảm Xúc</h3><br><kbd>Bạn đang thực hiện việc chỉnh sửa ID ở Sever 2</kbd>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
