<?php
    if(isset($_GET['id_like'])){
        ...
        $layinfo = mysqli_query($conn,"SELECT user_id,name,end,id_ctv FROM vipsv2 WHERE id=$id_like");
        $info = mysqli_fetch_assoc($layinfo);
        $uid = $info['user_id'];
        if($rule != 'admin'){
            if($idctv != $info['id_ctv']){
                echo "<script>alert('Bạn không có quyền gia hạn VIP ID này!');window.location='index.php?vip=Manager_VIP_Like_SV2';</script>";
            }else if($info['end'] > time()){
                echo "<script>alert('VIP ID này chưa hết hạn');window.location='index.php?vip=Manager_VIP_Like_SV2';</script>";
            }
        }else{
            if($info['end'] > time()){
                echo "<script>alert('VIP ID này chưa hết hạn');window.location='index.php?vip=Manager_VIP_Like_SV2';</script>";
...
if (isset($_POST['submit'])) {
    if ((($_POST['han'] != 'one' && $_POST['han'] != 'three') && $_POST['han'] !='seven') && (($_POST['han'] <= 0 || $_POST['han'] > 12) || $_POST['goi'] < 15000)) {
            echo "<script>alert('Địt con mẹ mày luôn, bug tiếp hộ bố mày cái ?'); window.location='index.php?vip=Manager_VIP_Like_SV2';</script>";
    } else {
        $loi = array();
        if(!isset($_POST['type'])){
        ...
        if (empty($loi)) {
            ...
            if($likes <= $max_like){
                ...
                if ($x['bill'] - $price >= 0) {
                    ...
                    $sql = "UPDATE vipsv2 SET han='$han', start='$start', end='$end', likes='$likes', max_like='$max_like', pay='$price',type='$type' WHERE id=$id_like";
                        ...
                        if (mysqli_query($conn, $up)) {
                            ...
                            if (mysqli_query($conn, $minus)) {
                                ...
                                if (mysqli_query($conn, $his)) {
                                    echo '<script>alert("Gia hạn thành công"); window.location="index.php?vip=Manager_VIP_Like_SV2";</script>';
                                }
                            }
                        }
                    }
                } else {
                    echo '<script>alert("Số dư tài khoản của bạn không đủ !!! Vui lòng nạp thêm tiền đi nha!1");location.href="them-index.php?vip=Manager_VIP_Like_SV2";</script>';
                }
            }else{
                echo "<script>alert('Vui lòng chọn số CX/Cron nhỏ hơn hoặc bằng gói CX');window.location='them-index.php?vip=Manager_VIP_Like_SV2';</script>";
            }
        }
    }
}
