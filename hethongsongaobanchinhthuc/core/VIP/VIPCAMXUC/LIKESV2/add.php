...
if (isset($_POST['submit'])) {
    ...
    if ($han <= 0 || $han > 12) { 
        ...
        if ($x['COUNT(user_id)'] > 0) {
            ...
            $loi['err'] = '<font color="red">User ID này đã tồn tại trên hệ thống tại sever <b>1</b></font>';
        }
        ...
        if (empty($loi)) {
            ...
            if ($likes <= $max_like) {
                ...
                $x = mysqli_fetch_assoc($result);
                if ($x['bill'] - $price >= 0) {
                    $sql = "INSERT INTO vipsv2(user_id, name, han, start, end, likes, max_like, id_ctv, pay,type, limitpost) VALUES('$uid','$name','$han','$start','$end','$likes','$max_like','$idctv','$price','$type', '$limitpost')";
                    if (mysqli_query($conn, $sql)) {
                        ...
                        if (mysqli_query($conn, $up)) {
                            ...
                            if (mysqli_query($conn, $minus)) {
                                if ($han == 'oneeee') {
                                    $content = "<b>$uname</b> vừa thêm VIP Cảm Xúc cho ID <b>$uid</b> tại sever <b>1</b>. Thời hạn <b>1 ngày </b> tháng, gói <b>$max_like</b> CX, Loại CX: $type, tổng thanh toán <b>" . number_format($price) . " VNĐ </b>";
                                } else if ($han == 'threeeeee') {
                                    $content = "<b>$uname</b> vừa thêm VIP Cảm Xúc cho ID <b>$uid</b> tại sever <b>1</b>. Thời hạn <b>3 ngày </b> tháng, gói <b>$max_like</b> CX, Loại CX: $type, tổng thanh toán <b>" . number_format($price) . " VNĐ </b>";
                                } else if ($han == 'sevennnnn') {
                                    $content = "<b>$uname</b> vừa thêm VIP Cảm Xúc cho ID <b>$uid</b> tại sever <b>1</b>. Thời hạn <b>7 ngày </b> tháng, gói <b>$max_like</b> CX, Loại CX: $type, tổng thanh toán <b>" . number_format($price) . " VNĐ </b>";
                                } else if (!empty(trim($_POST['coupon']))){
                                    $coupon = $_POST['coupon'];
                                    $get = mysqli_query($conn, "SELECT sale_off,COUNT(*) FROM coupon WHERE code = '$coupon' GROUP BY sale_off");
                                    $cop = mysqli_fetch_assoc($get);
                                    $content = "<b>$uname</b> vừa thêm VIP Cảm Xúc cho ID <b>$uid</b> tại sever <b>1</b>. Thời hạn <b>$han</b> tháng, gói <b>$max_like</b> CX, Loại CX: $type, Sử dụng mã giảm giá <b>".$coupon."(".$cop['sale_off']." %) tổng thanh toán <b>" . number_format($price) . " VNĐ </b>";
                                }else{
                                    $content = "<b>$uname</b> vừa thêm VIP Cảm Xúc cho ID <b>$uid</b> tại sever <b>1</b>. Thời hạn <b>$han</b> tháng, gói <b>$max_like</b> CX, Loại CX: $type, tổng thanh toán <b>" . number_format($price) . " VNĐ </b>";
                                }
                                $time = time();
                            ...
                            $id = $infouser['idbot'];
                            sendnoti($id,'Bạn vừa thêm VIP Cảm Xúc cho ID '.$uid.' tại sever 2. Thời hạn '.$han.' tháng, gói '.$max_like.' CX, tổng thanh toán '.number_format($price).' VNĐ');
                            /*End Get Thông Tin và gửi về tn facebook*/                                    echo '<script>alert("Thêm thành công"); window.location="index.php?vip=Add_VIP_Like_SV2";</script>';
                                }
                            }
                        }
                    }
                } else {
                    echo '<script>alert("Số dư tài khoản của bạn không đủ !!! Vui lòng nạp thêm tiền đi nha!1");location.href="index.php?vip=Add_VIP_Like_SV2";</script>';
                }
            } else {
                echo "<script>alert('Vui lòng chọn số CX/Cron nhỏ hơn hoặc bằng gói CX');window.location='index.php?vip=Add_VIP_Like_SV2';</script>";
            }
        }
    }
}
if($getsetting['viplike2'] == 'on'){
?>
<div class="row">
    <div class="col-md-12">
        <!-- Horizontal Form -->
        <div class="box box-info wow fadeIn">
            <div class="box-header with-border">
                <h3 class="box-title">Thêm ID VIP Cảm Xúc </h3><br> <kbd>Bạn đang thực hiện việc thêm ID ở Sever 2</kbd>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
    ...
    <div class="col-md-12">
        ...
        <div class="box box-info wow fadeIn">
            ...
            <div class="panel body">
                <center>
                    <b>
                        <p>Hiện tại sever 2 đã đủ ID vui lòng thêm id tại sever 1</p>
                        <p>Truy cập sever 1 <a href="/index.php?vip=Add_VIP_Like_SV1" target="_blank">SEVER VIP LIKE 1</a></p>
                    </b>
                </center>
            </div>
            </div>
